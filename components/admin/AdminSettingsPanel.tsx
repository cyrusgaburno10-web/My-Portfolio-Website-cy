  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/admin/settings', {
        ...
        body: JSON.stringify({ calendlyUrl, contactDestinationEmail, aiProvider, aiModel }),
      });
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-2xl border border-line bg-void-deep/40 p-6">
        {FIELDS.map((field) => (
          <div key={field.key} className="flex flex-col gap-1.5">
            <label className="font-mono text-[11px] uppercase tracking-[0.14em] text-periwinkle">{field.label}</label>
            <input
              type={field.type}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="font-mono text-[11px] uppercase tracking-[0.14em] text-periwinkle">AI chat provider</label>
          <select
            value={aiProvider}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="font-mono text-[11px] uppercase tracking-[0.14em] text-periwinkle">
